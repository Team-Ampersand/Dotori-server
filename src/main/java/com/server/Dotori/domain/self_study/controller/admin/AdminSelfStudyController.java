package com.server.Dotori.domain.self_study.controller.admin;
...
public class AdminSelfStudyController {
    ...
    public SingleResult getSelfStudyStudentByMemberNameAdmin(@RequestParam(value = "memberName") String memberName) {
    ...
    public SingleResult getSelfStudyStudentsByCreateDateAdmin() {
     ...
     * @author 조재영
     */
    @PatchMapping("/selfstudy/check/{memberId}")
    @ResponseStatus(HttpStatus.OK)
    ...
    @ResponseStatus(HttpStatus.OK)
    @ApiOperation(value = "자습 내려갔는지 확인취소", notes = "자습 내려갔는지 확인취소")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "Authorization", value = "로그인 성공 후 access_token", required = true, dataType = "String", paramType = "header"),
            @ApiImplicitParam(name = "RefreshToken", value = "로그인 성공 후 refresh_token", required = false, dataType = "String", paramType = "header")
    })
